...
bool linReg(std::string filename, int run, const int hits,
  ...
  double fExpHall_x = (cubeNum * voxelThicc) + ((cubeNum + 1) * voxelSpace);
  double fExpHall_y = (cubeNum * voxelDim) + ((cubeNum + 1) * voxelSpace)
    + (((cubeNum-1)/2.) * (voxelDim+voxelSpace));
  double fExpHall_z = (cubeNum * voxelDim) + ((cubeNum + 1) * voxelSpace)
    + (((cubeNum-1)/2.) * (voxelDim+voxelSpace));
  ...
  rSquaredTheta = 1 - (numerTheta/denomTheta);
  rSquaredPhi = 1 - (numerPhi/denomPhi);
