...
const int ROWS = 1e2; //1e6
bool doubleSiPM = 1;
bool inlin3 = 1;
bool fixDS = 1; //reorders columns because I'm mentally deficient 

const int COLS = cubeNum; //15
...
bool linReg(std::string filename, int run, const int hits,
  ...
  //G4 variables
  int CPNo, i, j;
  double voxelDim = 1., voxelThicc = 16., voxelSpace = .01;
  double SiPMDimy = .1;
  double p0 = .336; //SiPMDR = tanh(p0 * jpos)
  ...
  if(inlin3){//for inline orientation
    fExpHall_x = (voxelThicc) + ((cubeNum + 1) * voxelSpace)
      + (SiPMDimy * 2.);
    fExpHall_y = (cubeNum * voxelDim) + ((cubeNum + 1) * voxelSpace);
    if(cubeNum > 1){
      fExpHall_y += (SiPMDimy * 2.);
    }
  ...
  for(n = 0; n < hits; ++n){
    ...
    if(inlin3){//inline
      if(i%2 == 1){//rotate
	ipos[n] = -(fExpHall_x/2.) + voxelSpace + (voxelDim/2.)
	  + (j * (voxelDim+voxelSpace));
	jpos[n] = 0;
	kpos[n] = -(fExpHall_z/2.) + voxelSpace + (voxelDim/2.)
	  + (i * (voxelDim+voxelSpace));
      }
      else{
	ipos[n] = 0;
    ...
    /*
      std::cout << "(i,j): (" << i << "," << j << ")\n";
      std::cout << "(x,y,z): (" << ipos[n] << "," << jpos[n] << "," << kpos[n];
      std::cout << ")\n";*/
	...
	if(std::abs(SiPMDifferenceRatio) == 1){
	  jpos[n] += SiPMDifferenceRatio * (voxelThicc/2.);
	}
	else{
	...
	if(std::abs(SiPMDifferenceRatio) == 1){
	  ipos[n] += SiPMDifferenceRatio * (voxelThicc / 2.);
	}
	else{
	  ipos[n] += std::atanh(SiPMDifferenceRatio) / p0;
	}
      }
    }
