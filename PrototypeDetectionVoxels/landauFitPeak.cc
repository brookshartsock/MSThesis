...
bool pTube = 1; // 0 for SiPM (positive landau), 1 for Ptube (negative)
...
peakData landauFit(int fileNo, double timEst) {
  ...
  while((timeVec[k] < timEst) && (k < dataSize)){
    k +=1;
  }
  ...
  graph->Fit("landauFunc", "Q", "", timeVec[j - 20], timeVec[k + 40]); 
...
int main(){
  ...
  std::fstream outfile;
  outfile.open(outputFilename.c_str(), std::fstream::in | std::fstream::out | std::fstream::trunc);

  outfile << "event,peak,timing\n";

  int i = 0;
  int iMax = 1e6; //1e6
  bool check = 1, isOldData = 0; //for the loop and to check for data structure
  while(check && i < iMax){ // i = 0; i < files
    ...
    if(check){
      ...
      thispeak = landauFit(i, timEst);

      i += 1;
      outfile << i << "," << thispeak.peak <<  "," << thispeak.timing << "\n";
    }
    else{
      if(!isOldData){
	isOldData = 1;
