...
double lFitCh2Data(int fileNo){
  ...
  double ch2Slope = .4599; //these are for 2x
  double ch2SlopeErr = .01768;
  double ch2Intercept = .008531;
  double ch2InterceptErr = .008331;
...
double lFitCh1Data(int fileNo){
  ...
  double ch1Slope = .3653; //these are for 1x
  double ch1SlopeErr = .01657;
  double ch1Intercept = .01305;
  double ch1InterceptErr = .006564;
...
int main(){
  ...
  outfile << "tek#,ch2Edep(MeV),ch2EdepErr(MeV),ch1Edep(MeV),ch1EdepErr(MeV),";
  outfile << "time(ns)\n";
  ...
  while(check && (i < iMax)){ // i = 0; i < files
	...
	zeroData();
	double ch2PeakTime = lFitCh2Data(i)*1e9;
	//std::cout << ch2PeakTime << "\n";
	double ch1PeakTime = lFitCh1Data(i)*1e9;
	//std::cout << ch1PeakTime << "\n";

	dataString += std::to_string(ch2PeakTime-ch1PeakTime) + "\n";
