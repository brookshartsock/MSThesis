...
LXePMTSD::LXePMTSD(G4String name)
  ...
  ,fPMTPositionsY(0),fPMTPositionsZ(0), pmtCount0(0), pmtCount1(0)
{
  collectionName.insert("pmtHitCollection");
  /*
  G4AnalysisManager* analysisManager = G4AnalysisManager::Instance();
  analysisManager->SetNtupleMerging(true); // Enable ntuple merging

  // Create an ntuple with columns for PMT number and photon count
  analysisManager->CreateNtuple("PMTData", "PMTData");
...
G4bool LXePMTSD::ProcessHits_constStep(const G4Step* aStep,
  ...
  if(pmtNumber == 0){
    ...
    for(int i = 0; i < 10; ++i){
      std::cout << "SOMETHING BROKE SOMETHING BROKE SOMETHING BROKE\n";
    }
  }
...
void LXePMTSD::EndOfEvent(G4HCofThisEvent* ) {
  ...
  //std::cout << "pmtCount0 = " << pmtCount0 << " photons\n";
  //std::cout << "pmtCount1 = " << pmtCount1 << " photons\n";

  std::fstream outfile;
  outfile.open("doubleSiPMData.csv", std::fstream::in | std::fstream::out 
	       | std::fstream::app);
  outfile << pmtCount0 << "," << pmtCount1  << "\n";
  outfile.close();
