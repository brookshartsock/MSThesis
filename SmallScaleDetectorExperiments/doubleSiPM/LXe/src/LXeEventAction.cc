...
void LXeEventAction::EndOfEventAction(const G4Event* anEvent){
  ...
  if(pmtHC){
    ...
    if(eventInformation->GetHitCount()>0){//dont bother unless there were hits
      //std::cout << "Collected here? " << eventInformation->GetHitCount() << "\n";
  ...
  /*
  LXeDetectorConstruction::voxelSpecifics EventAction;
  G4int cubeNum = EventAction.voxelNum;
  std::fstream outfile;
  outfile.open("data.csv", std::fstream::in | std::fstream::out 
	       | std::fstream::app);
  outfile << cubeNum*cubeNum*cubeNum << "\n";
  outfile.close();*/

  //--------------------------------

  //LXeSteppingVerbose* mySteppingVerbose =
  //(LXeSteppingVerbose*)G4RunManager::GetRunManager()->GetUserSteppingVerbose();
  /*
  //if (mySteppingVerbose) { // Check if it's not a nullptr
  LXeSteppingVerbose* steppingVerbose = new LXeSteppingVerbose;
  //G4cout << "test" << G4endl;
  int eventData = steppingVerbose->GetStepData();//causing dump
  //G4cout << "test" << G4endl;
    std::cout << "OUTPUT TEST HERE: " << eventData << "\n";
    //}
