                 ...
                 "housing",pMotherLogical,pMany,pCopyNo),fConstructor(c)
{
  ...
  if(orientation){//for inline orientation
    housing_x= (voxelThicc) + ((cubeNum + 1) * voxelSpace)
      + (SiPMDimy * (doubleSiPM+1)) + 3.;
    housing_y= (cubeNum * voxelDim) + ((cubeNum + 1) * voxelSpace) + 3.;
    if(cubeNum > 1){
      housing_y += (SiPMDimy * (doubleSiPM+1));
    }
    housing_z= (cubeNum * voxelDim) + ((cubeNum + 1) * voxelSpace) + 3.;
  }
  ...
  fAluminum_box =  new G4Box("Aluminum", (voxelThicc+AlDim)/2, 
			     (voxelDim+AlDim-.4)/2, (voxelDim+AlDim)/2);//cm
  ...
  fScint_box = new G4Box("scint_box", voxelThicc/2, (voxelDim-.4)/2, voxelDim/2);
  ...
  */
  ...
  double SiPMDim = 6.;
  fPmt = new G4Box("pmt_tube", SiPMDimy/2., SiPMDim/2., SiPMDim/2.);

  fPmt_log = new G4LogicalVolume(fPmt,G4Material::GetMaterial("Glass"),
                                 "pmt_log");

  new G4PVPlacement(0, G4ThreeVector(-.5*(voxelThicc+SiPMDimy)+.1,0,0), 
  fPmt_log, "pmt_tube", fScint_log, false, 0);
  new G4PVPlacement(0, G4ThreeVector(.5*(voxelThicc+SiPMDimy)-.1,0,0), 
  fPmt_log, "pmt_tube", fScint_log, false, 1);

  //the "photocathode" is a metal slab at the back of the glass that
  //is only a very rough approximation of the real thing since it only
  //absorbs or detects the photons based on the efficiency set below
  fPhotocath = new G4Box("photocath_tube", SiPMDimy/2., SiPMDim/2., SiPMDim/2);
