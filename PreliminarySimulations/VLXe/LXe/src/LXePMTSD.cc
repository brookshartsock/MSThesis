...
LXePMTSD::LXePMTSD(G4String name)
  ...
  ,fPMTPositionsY(0),fPMTPositionsZ(0)//,SiPMCount0{0},SiPMCount1{0}
{
...
G4bool LXePMTSD::ProcessHits_constStep(const G4Step* aStep,
  ...
  G4int pmtNumber =
    aStep->GetPostStepPoint()->GetTouchable()->GetReplicaNumber(3);
  G4int SiPMNumber =
    aStep->GetPostStepPoint()->GetTouchable()->GetReplicaNumber(1);
  G4VPhysicalVolume* physVol=
    aStep->GetPostStepPoint()->GetTouchable()->GetVolume(1);

  //G4cout << "WORKING WORKING WORKING WORKING";
  
  LXeDetectorConstruction::voxelSpecifics PMTSD;
  bool photonsCounting = PMTSD.countPhotons;
  ...
  if(photonsCounting){
    //std::cout << "WORKING";
    if(SiPMNumber == 0){
      ++SiPMCount0[pmtNumber];
    }
    else if(SiPMNumber == 1){
      ++SiPMCount1[pmtNumber];
    }
  }
...
void LXePMTSD::EndOfEvent(G4HCofThisEvent* ) {
  ...
  LXeDetectorConstruction::voxelSpecifics PMTSD;
  int cubeNum = PMTSD.voxelNum;

  if(PMTSD.countPhotons){
  
    std::fstream outfile;
    outfile.open("data.csv", std::fstream::in | std::fstream::out 
		 | std::fstream::app);
  
    for(int i = 0; i < (cubeNum*cubeNum*cubeNum); ++i){
    
      SiPMDiffRatio[i] = ((double)SiPMCount1[i] -(double) SiPMCount0[i])
	/ ((double)SiPMCount1[i] + (double)SiPMCount0[i]);
      if(std::to_string(SiPMDiffRatio[i]) != "-nan"){
	outfile << SiPMDiffRatio[i]/4. + i << ",";
      }
