...
/*
LXeDetectorConstruction::voxelSpecifics LXeDetectorConstruction::GetVoxelSpecifics(){

  LXeDetectorConstruction::voxelSpecifics set;
  set.voxelDim = 7.*mm;
  set.voxelThicc = 1.*mm;
  set.voxelSpace = 2.*mm;
  set.voxelNum = 6;
...
void LXeDetectorConstruction::DefineMaterials(){
  ...
  //"Ew gross!" (GaGG)
  fGd = new G4Element("Gadolinium", "Gd", z = 64, a = 157.2*g/mole);
  fGa = new G4Element("Gallium", "Ga", z = 31, a = 69.72*g/mole);
  fal = new G4Element("aluminum", "al", z = 13, a = 26.98*g/mole);
  fGAGGCe = new G4Material("GAGGCe",density=6.63*g/cm3,4);
  fGAGGCe -> AddElement(fGd, 3);
  fGAGGCe -> AddElement(fal, 2);
  fGAGGCe -> AddElement(fGa, 3);
  fGAGGCe -> AddElement(fO, 12);
  ...
  fPethylene2->SetMaterialPropertiesTable(clad2Property);
    ...
    G4double photonEnergyGAGG[] =
            {1.2398/0.400*eV , 1.2398/0.410*eV , 1.2398/0.420*eV , 
	     1.2398/0.430*eV , 1.2398/0.440*eV , 1.2398/0.450*eV ,
	     1.2398/0.460*eV , 1.2398/0.470*eV , 1.2398/0.480*eV , 
	     1.2398/0.490*eV , 1.2398/0.500*eV , 1.2398/0.510*eV ,
	     1.2398/0.520*eV , 1.2398/0.530*eV , 1.2398/0.540*eV , 
	     1.2398/0.550*eV , 1.2398/0.560*eV , 1.2398/0.570*eV ,
	     1.2398/0.580*eV , 1.2398/0.590*eV , 1.2398/0.600*eV , 
	     1.2398/0.610*eV , 1.2398/0.620*eV , 1.2398/0.630*eV ,
	     1.2398/0.640*eV , 1.2398/0.650*eV };

 const G4int nEntriesGAGG = sizeof(photonEnergyGAGG)/sizeof(G4double);
 ...
 G4double refractiveIndexGAGG[] =
  ...
  assert(sizeof(refractiveIndexGAGG) == sizeof(photonEnergyGAGG));

  G4double absorptionGAGG[] =
           { 645.* mm, 645.* mm, 645.* mm, 645.* mm, 645.* mm, 645.* mm,
	     645.* mm, 645.* mm, 645.* mm, 645.* mm, 645.* mm, 645.* mm,
	     645.* mm, 645.* mm, 645.* mm, 645.* mm, 645.* mm, 645.* mm,
	     645.* mm, 645.* mm, 645.* mm, 645.* mm, 645.* mm, 645.* mm,
	     645.* mm, 645.* mm, };

  assert(sizeof(absorptionGAGG) == sizeof(photonEnergyGAGG));

  G4double scintilFastGAGG[] =
            { 0., 0., 0., 0., 0., 0., 0., 0.02, 0.13, 0.385, 
	      0.766, 0.95, 1.00, 0.96, 0.896, 0.814, 0.716, 
	      0.62, 0.51, 0.42, 0.34, 0.288, 0.242, 0.211, 0.185, 0.162};

  assert(sizeof(scintilFastGAGG) == sizeof(photonEnergyGAGG));

  G4MaterialPropertiesTable* myMPTGAGG = new G4MaterialPropertiesTable();

  myMPTGAGG->AddProperty("RINDEX", photonEnergyGAGG, 
			 refractiveIndexGAGG,nEntriesGAGG)
        ->SetSpline(true);
  myMPTGAGG->AddProperty("ABSLENGTH",    photonEnergyGAGG, 
			 absorptionGAGG,     nEntriesGAGG)
        ->SetSpline(true);
  myMPTGAGG->AddProperty("FASTCOMPONENT",photonEnergyGAGG, 
			 scintilFastGAGG, nEntriesGAGG)
        ->SetSpline(true);

  myMPTGAGG->AddConstProperty("SCINTILLATIONYIELD",420./MeV);
  myMPTGAGG->AddConstProperty("RESOLUTIONSCALE",1.0);
  myMPTGAGG->AddConstProperty("FASTTIMECONSTANT", 80.*ns);
  myMPTGAGG->AddConstProperty("YIELDRATIO",1.0);

  fGAGGCe->GetIonisation()->SetBirksConstant(0.115*mm/MeV);

  fGAGGCe->SetMaterialPropertiesTable(myMPTGAGG);
...
G4VPhysicalVolume* LXeDetectorConstruction::ConstructDetector()
{
  ...
  LXeDetectorConstruction::voxelSpecifics DetectorConstruction;

  G4double voxelDim = DetectorConstruction.voxelDim;
  G4double voxelThicc = DetectorConstruction.voxelThicc;
  G4double voxelSpace = DetectorConstruction.voxelSpace;
  G4int cubeNum = DetectorConstruction.voxelNum;
  bool orientation = DetectorConstruction.inlin3;

  G4double expHall_x;
  G4double expHall_y;
  G4double expHall_z;
  
  //The experimental hall walls are all 1cm away from housing walls
  if(orientation){
    expHall_x = (cubeNum * voxelThicc) + 
      ((cubeNum + 1) * voxelSpace)+1.*cm;
    expHall_y = (cubeNum * voxelDim) + ((cubeNum + 1) * voxelSpace) 
      + 1.*cm;
    expHall_z = (cubeNum * voxelDim) + ((cubeNum + 1) * voxelSpace) 
      + 1.*cm;
  }
  else{
    expHall_x = (cubeNum * voxelThicc) + 
      ((cubeNum + 1) * voxelSpace)+1.*cm;
    expHall_y = (cubeNum * voxelDim) + ((cubeNum + 1) * voxelSpace) 
      + (((cubeNum-1)/2.) * (voxelDim+voxelSpace)) + 1.*cm;
    expHall_z = (cubeNum * voxelDim) + ((cubeNum + 1) * voxelSpace) 
      + (((cubeNum-1)/2.) * (voxelDim+voxelSpace)) + 1.*cm;
  }
