...
/*
  LXeMainVolume::voxelSpecifics LXeMainVolume::GetVoxelSpecifics(){

  LXeMainVolume::voxelSpecifics set;
  set.voxelDim = 7.*mm;
  set.voxelThicc = 1.*mm;
  set.voxelSpace = 2.*mm;
  set.voxelNum = 6;
                 ...
                 "housing",pMotherLogical,pMany,pCopyNo),fConstructor(c)
{
  ...
  int SiPMCount = MainVolume.doubleSiPM + 1;
  //bool reflectionsOfPhotons = MainVolume.photonReflections;
  ...
  //aluminum
  G4double ipos = -(housing_x/2.) + voxelSpace + (voxelThicc/2.);
  G4double jpos = -(housing_y/2.) + voxelSpace + (voxelDim/2.);
  G4double kpos = -(housing_z/2.) + voxelSpace + (voxelDim/2.);
